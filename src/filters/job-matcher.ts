...
export class JobMatcher {
  ...
  private calculateKeywordScore(job: JobListing, keywords: string): number {
  ...
  private calculateLocationScore(job: JobListing, preferredLocation: string): number {
  ...
  private calculateJobTypeScore(job: JobListing, preferredType: string): number {
  ...
  private calculateExperienceScore(job: JobListing, preferredLevel: string): number {
    ...
    const keywords = levelKeywords[preferredLevel as keyof typeof levelKeywords] || [];
    let matches = 0;
  ...
  private calculateSkillsScore(job: JobListing): number {
  ...
  filterJobs(jobs: JobListing[], criteria: JobCriteria, minScore: number = 60): JobListing[] {
    return jobs
      .map(job => ({
        job,
  ...
  getMatchingJobs(jobs: JobListing[], criteria: JobCriteria, limit: number = 10): Array<{ job: JobListing; score: number }> {
  ...
  extractSalaryRange(job: JobListing): { min?: number; max?: number } | null {
  ...
  isWithinSalaryRange(job: JobListing, criteria: JobCriteria): boolean {
    if (!criteria.salaryRange) return true;
    ...
    if (min && jobSalary.max && jobSalary.max < min) return false;
    if (max && jobSalary.min && jobSalary.min > max) return false;
